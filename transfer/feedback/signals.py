...
def send_tele_bot_and_email(feedback, bot_settings):
    message = MESSAGE.format(feedback.name, feedback.email, feedback.telephone,
                             feedback.message, feedback.datetime_create.strftime('%d/%m/%Y %H:%M'))
    try:
    ...
    send_mail_valid = send_mail('Регистрация на сайте', message, settings.EMAIL_HOST_USER, [
...
def send_telegramm(sender, instance, created, **kwargs):
    if created:
        ...
        transaction.on_commit(
            lambda: send_tele_bot_and_email(instance, telegram_bot))
