...
def send_email_invite_recall(sender, instance, **kwargs):
	if instance.performed and instance.in_work:
		...
		if not recall_invite:
			...
			message_mail = INVITE_MESSAGE.format(settings.SITE, recall_invite.id)
			send_mail_valid = send_mail('Регистрация на сайте', message_mail, settings.EMAIL_HOST_USER, [instance.email], fail_silently=True)
			if send_mail_valid:
				recall_invite.save()